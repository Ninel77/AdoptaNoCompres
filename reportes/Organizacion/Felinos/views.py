...
def GATrepTotalOrg(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion

    tAni = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='FELINA').filter(
        especie__especies='FELINA').count()
    tAnIngre = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='FELINA').filter(estado='REHABILITACION').count()
    tAnEnAdop = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='FELINA').filter(estado='EN ADOPCION').count()
    tAnAdop = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='FELINA').filter(estado='ADOPTADO').count()
    tAnEnSant = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='FELINA').filter(estado='SANTUARIO').count()

    tTamP = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='FELINA').filter(
        tamano='PEQUEÑO').count()
    tTamM = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='FELINA').filter(
        tamano='MEDIANO').count()
    tTamG = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='FELINA').filter(
        tamano='GRANDE').count()
    tTamGG = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='FELINA').filter(tamano='GIGANTE').count()

    tSexF = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='FELINA').filter(
        sexo='HEMBRA').count()
    tSexM = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='FELINA').filter(
        sexo='MACHO').count()

    tGestIn = AnimalesRescatados.objects.filter(animal__organizacion__nomorganizacion=nOrg).filter(
        animal__especie__especies='FELINA').filter(
    ...
    return render(request, 'Reportes/Organizacion/detalles/Todo.html',
                  {
                      ...
                      'entity': 'Reporte acerca de la  Especie Felina (Gatos)',
                      'titulo': 'Reporte acerca de la  Especie Felina (Gatos)',
...
def GATrepRehabAnimOrg(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion
...
def GATrepEnAdopAnimOrg(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion
...
def GATrepEnSanAnimOrg(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion
