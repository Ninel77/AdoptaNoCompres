...
def repTotalOrg(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion

    tAni = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).count()
    tAnIngre = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(estado='REHABILITACION').count()
    tAnEnAdop = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(estado='EN ADOPCION').count()
    tAnAdop = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(estado='ADOPTADO').count()
    tAnEnSant = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(estado='SANTUARIO').count()

    tEspC = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies__icontains='CANINA').count()
    tEspF = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies__icontains='FELINA').count()
    tEspO = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies__icontains='OTRO').count()

    tTamP = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(tamano='PEQUEÑO').count()
    tTamM = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(tamano='MEDIANO').count()
    tTamG = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(tamano='GRANDE').count()
    tTamGG = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(tamano='GIGANTE').count()

    tSexF = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(sexo='HEMBRA').count()
    tSexM = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(sexo='MACHO').count()
    ...
    return render(request, 'Reportes/Organizacion/detalles/Todo.html',
                  {
                      ...
                      'entity': 'Reporte General acerca de los animales de compañia',
                      'titulo': 'Reporte General acerca de los animales de compañia',
                      ...
                      'tAnEnSant': tAnEnSant,
                      'tituloDos': 'Cantidad total de animales registrados segun tipo de Especie',
                      'tEspC': tEspC, 'tEspF': tEspF, 'tEspO': tEspO,
...
def RepOrgCantVol(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion

    V = Voluntario.objects.filter(organizacion__nomorganizacion=nOrg).count()
    f = Voluntario.objects.filter(organizacion__nomorganizacion=nOrg).filter(user__persona__Genero='FEMENINO').count()
    m = Voluntario.objects.filter(organizacion__nomorganizacion=nOrg).filter(user__persona__Genero='MASCULINO').count()
    act = Voluntario.objects.filter(organizacion__nomorganizacion=nOrg).filter(user__is_active=True).count()
    inact = Voluntario.objects.filter(organizacion__nomorganizacion=nOrg).filter(user__is_active=False).count()


    return render(request, 'Reportes/Organizacion/detalles/Org.html',
                  {
                      'nav': 'General',
                      'entity_anterior_url': reverse_lazy('rep:reportesOrganizacion:IndiceRepOrg'),
                      'entity_anterior': 'Lista de Reportes',
                      ...
                      'titulo': 'Reporte General acerca de los animales de compañia',
                      'tituloprincipal': 'Informacion General de los animales en Santuarios o Refugios de manera permanente',
                      'tituloUno': 'Cantidad Total de Voluntarios en la organizacion',
                      'V': V,
                      'tituloDos': 'Cantidad de voluntarios Activos/Inactivos',
                      'act':act,'inact':inact,
                      'tituloTres': 'Cantidad de voluntarios segun GENERO',
                      'F': f, 'M': m,
