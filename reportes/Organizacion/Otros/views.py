...
def OTRrepTotalOrg(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion

    tAni = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='OTRO').filter(
        especie__especies='OTRO').count()
    tAnIngre = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='OTRO').filter(estado='REHABILITACION').count()
    tAnEnAdop = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='OTRO').filter(estado='EN ADOPCION').count()
    tAnAdop = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='OTRO').filter(estado='ADOPTADO').count()
    tAnEnSant = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='OTRO').filter(estado='SANTUARIO').count()

    tTamP = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='OTRO').filter(
        tamano='PEQUEÑO').count()
    tTamM = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='OTRO').filter(
        tamano='MEDIANO').count()
    tTamG = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='OTRO').filter(
        tamano='GRANDE').count()
    tTamGG = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(
        especie__especies='OTRO').filter(tamano='GIGANTE').count()

    tSexF = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='OTRO').filter(
        sexo='HEMBRA').count()
    tSexM = AnimalesDatosB.objects.filter(organizacion__nomorganizacion=nOrg).filter(especie__especies='OTRO').filter(
        sexo='MACHO').count()

    tGestIn = AnimalesRescatados.objects.filter(animal__organizacion__nomorganizacion=nOrg).filter(
        animal__especie__especies='OTRO').filter(
    ...
    return render(request, 'Reportes/Organizacion/detalles/Todo.html',
                  {
                      ...
                      'entity': 'Reporte acerca de la  Especie OTRO ',
                      'titulo': 'Reporte acerca de la  Especie OTRO ',
...
def OTRrepRehabAnimOrg(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion
...
def OTRrepEnAdopAnimOrg(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion
...
def OTRrepEnSanAnimOrg(request):
    nOrg = request.user.voluntario_perfil.organizacion.nomorganizacion
